...
<div class="form-group">
    ...
    <div class="input-group">
        ...
        <input id="{{ $id OR '' }}" name="{{ $name }}" class="{{ $class OR '' }} {{ $errors->has($name) ? 'is-invalid' : '' }}" placeholder="{{ $placeholder OR '' }}" type="text" value="{{ $value ?? '' }}" @echoIf('readonly', $readonly ?? false) @echoIf('required', $required ?? false) }}>
